...
def preprocessing(setname='train', ROOT_PATH='../../../materials', data_aug=False):
    ...
    for i in range(lb+1):
        ...
        total = (dt / (i+1)) * 64
        print('{} class done: {}, time: {:.2f}/{:.2f}'.format(i+1, cls_img.size(), dt, total))
        if i == 0:
            data_ten = cls_img.unsqueeze(0)
        else:
            data2 = cls_img.unsqueeze(0)
            data_ten = torch.cat((data_ten, data2)) # [64, 600, 50, 3, 84, 84]
    torch.save(data_ten, 'train-50.pt')


preprocessing(setname='train', ROOT_PATH='../../../materials', data_aug=True)
